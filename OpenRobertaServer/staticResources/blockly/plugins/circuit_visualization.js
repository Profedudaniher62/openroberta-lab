...
* @license ISC
*
* BUILT: Mon Sep 14 2020 11:01:37 GMT+0200 (Central European Summer Time)
*/;
var CircuitVisualization = (function () {
	...
	*/
	...
	var microbit = [{
	  ...
	  name: 'robConf_temperature',
	  position: {
	    x: 262,
	    y: 48.2
	  }
	}, {
	  name: 'robConf_accelerometer',
	  position: {
	    x: 238.2,
	    y: 99.5
	  }
	}, {
	  name: 'robConf_compass',
	  position: {
	    x: 238.2,
	    y: 104.7
	  }
	...
	var WirePoint = /*#__PURE__*/function () {
	  function WirePoint(position) {
	    _classCallCheck(this, WirePoint);

	    this.position_ = position;
	    this.next = null;
	  }

	  _createClass(WirePoint, [{
	    key: "position",
	    get: function get() {
	      return this.position_;
	    }
	...
	var WireDrawer = /*#__PURE__*/function () {
	  function WireDrawer(origin, destination) {
	    _classCallCheck(this, WireDrawer);
	  ...
	  _createClass(WireDrawer, [{
	    key: "addPoint_",
	    value: function addPoint_(prevPoint, position) {
	    ...
	    key: "toOrthoLines_",
	    value: function toOrthoLines_() {
	      var _this$head_$position = this.head_.position,
	          x1 = _this$head_$position.x,
	          y1 = _this$head_$position.y;
	      var _this$head_$next$posi = this.head_.next.position,
	          x2 = _this$head_$next$posi.x,
	          y2 = _this$head_$next$posi.y;
	      if (x1 === x2 || y1 === y2) return;
	      var x = x1 < x2 ? lodash.max([x1, x2]) : lodash.min([x1, x2]);
	      var y = y1 < y2 ? lodash.min([y1, y2]) : lodash.max([y1, y2]);
	      this.addPoint_(this.head_, {
	        x: x,
	        y: y
	      });
	    }
	    ...
	    get: function get() {
	      ...
	      var moveto = this.head_.position;

	      var path = concat$2(_context = "M ".concat(moveto.x, " ")).call(_context, moveto.y);
	      ...
	      while (current !== null) {
	        ...
	        path = concat$2(_context2 = concat$2(_context3 = "".concat(path, " L ")).call(_context3, lineto.x, " ")).call(_context2, lineto.y);
	...
	var CircuitVisualization = /*#__PURE__*/function () {
	  _createClass(CircuitVisualization, null, [{
	    ...
	    value: function domToWorkspace(dom, workspace) {
	      ...
	      if (workspace.device !== 'arduino' && workspace.device !== 'microbit') {
	  ...
	  function CircuitVisualization(workspace, dom) {
	    ...
	    this.renderConnections_ = function () {
	      ...
	      forEach$4(_context5 = _this.connections_).call(_context5, function (_ref) {
	        ...
	        var drawer = new WireDrawer(origin, destination);
	    ...
	    this.createBlockPorts_ = function (block) {
	      ...
	      forEach$4(_context8 = block.inputList).call(_context8, function (input, index) {
	        if (index === 0) {
	          ...
	          if (_this.robot_.getPortByName(type)) {
	            _this.appendPortAndConnection_(block, input.fieldRow[1].fieldGroup_, name, type);
	          }
	        } else {
	          var _context9;
	          ...
	          forEach$4(_context9 = input.fieldRow).call(_context9, function (_ref3) {
	            ...
	            if (name) {
	              ...
	              var connectedTo = fixPortValue((_block$getFieldValue = block.getFieldValue(name)) !== null && _block$getFieldValue !== void 0 ? _block$getFieldValue : value_);
	    ...
	    this.appendPortAndConnection_ = function (block, fieldGroup, name, connectedTo) {
	      var _DEFAULT_COLORS$get;

	      var _fieldGroup$transform = fieldGroup.transform.baseVal.getItem(0),
	          matrix = _fieldGroup$transform.matrix;
